...
`--backend qemu`. See [Providers](/docs/learn/concepts/providers) for the backend
distinction.
...
CPU count, RAM, storage, GPU count, GPU kind, and the per-device GPU list all come
back from the agent rather than from what you typed. Two values are instead
**operator-declared**, because they are policy or pre-created state rather than
hardware facts the agent can observe: `--max-vms` is a ceiling you choose, and
