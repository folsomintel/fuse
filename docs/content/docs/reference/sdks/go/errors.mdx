...
provider has no guest to [`Exec`](/docs/reference/sdks/go/environments) or
[`Attach`](/docs/reference/sdks/go/environments#interactive-sessions) into),
with no matching `Is*` helper yet, check `apiErr.Code == "unimplemented"`
