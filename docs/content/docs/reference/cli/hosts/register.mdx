...
Each profile is validated against the mig-parted grammar and must carry a positive
count, so a typo fails at registration rather than silently advertising capacity
that can never be filled. Malformed values are rejected with
`invalid --mig-profile "...": expected profile=count (e.g. 1g.10gb=4)`, and a
non-positive count with `invalid --mig-profile "...": count must be a positive
number`. Profile keys are lowercased.
