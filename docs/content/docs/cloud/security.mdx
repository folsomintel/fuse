...
users, no roles. See [Authentication](/docs/reference/auth) for the full model.
...
per-VM guest-agent tokens at rest with AES-256-GCM before they're written to
the state store. Losing this key doesn't lock you out of the orchestrator
itself, but it does mean previously-issued per-VM tokens can no longer be
decrypted, rotate every environment's token afterward via
[`fuse environment rotate-token`](/docs/reference/cli/environment/rotate-token)
if you ever have to regenerate it.

## Master token vs. API keys

Prefer revocable [API keys](/docs/reference/cli/apikeys) over the master
...
[Authentication](/docs/reference/auth#browser--session-auth) for the full
cookie model.
