...
Upgrade with `brew upgrade --cask folsomintel/fuse/fuse`, and check the installed version
with `fuse --version`. On Linux without Homebrew, take the `fuse-cli_Linux_*.tar.gz`
archive from the [latest release](https://github.com/folsomintel/fuse/releases/latest).
