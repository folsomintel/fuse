...
// reports per-device inventory the request must be satisfiable by a device
// that is both MIGCapable and a kind match (gpuKindMatches semantics, same as
// the whole-device branch). Devices are read from Capacity and not from the
// free set: a GPU in MIG mode stays on the nvidia driver and is never part of
// the whole-device pool (D5), so whole-device allocations say nothing about
// MIG availability.
//
...
func hostServesMIG(capacity HostCapacity, gpuKind string) bool {
	if len(capacity.GPUDevices) == 0 {
		return gpuKind == "" || capacity.GPUKind == "" ||
			strings.EqualFold(gpuKind, capacity.GPUKind)
	}
